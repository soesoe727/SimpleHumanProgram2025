...
void  MotionPlaybackApp::Display()
{
	...
	if(curr_posture && curr_posture2)
	{
		...
		else
		{
			...
			{
				//2人目の動作の可視化(白･灰)(パス対応)
				curr_posture3 = motion2->frames[DTWa->DTWpass[view_segment][frame_no]];
			...
			{
				//2人目の動作の可視化(白･灰)(パス対応)
				curr_posture3 = motion2->frames[DTWa->DTWpass[view_segment][motion->num_frames - 1]];
				DrawPostureGray(curr_posture3, pattern, view_segment);
				DrawPostureShadow(curr_posture3, shadow_dir, shadow_color);
			}
		}
	...
	if ( motion && motion2 )
	{
		...
		{
			for(int i = 0; i < 5; i++)
				sprintf( messages[i], "%s : %.2f percent", motion->body->segments[DTWa->DTWorder[i]]->name.c_str(), 100 * DTWa->DTWdistance[DTWa->DTWorder[i]][motion->num_frames] / DTWa->DTWtotal);
		}
		else
		{
			DTWa->DTWinformation_frame(frame_no, motion2->body->num_joints);
			for(int i = 0; i < 5; i++)
				sprintf( messages[i], "%s : %.2f", motion->body->segments[DTWa->DTWorder[i]]->name.c_str(), DTWa->DTWdistance[DTWa->DTWforder[i]][frame_no]);
		}
		/*
		if( pattern == 0 )
...
void  MotionPlaybackApp::MouseDrag( int mx, int my )
{
	...
	if(selected_time < 0)
		selected_time = 0.0f;
	else if(selected_time > motion->num_frames)
		selected_time = motion->num_frames - 1;
...
{
	int Track_num = 0;
	float num_space = 30.0f; //部位の名前分のスペース
	
	// タイムラインの時間範囲を設定
	timeline->SetTimeRange( 0.0f, motion.num_frames + num_space );
	...
	switch(pattern)
	{
		case 0:
			...
			{
				ColorBarElement(timeline, j, Track_num, DTW->head_chest, motion);
			...
			ColorBarElement(timeline, 0, Track_num, DTW->head_chest, motion);//腰
			PartDepLine(timeline, 1, Track_num++, DTW, motion, pattern);
			Track_num++;
			for(int j = 13; j <= 16; j++)//右腕
			{
				ColorBarElement(timeline, j, Track_num, DTW->arm, motion);
			...
			{
				ColorBarElement(timeline, j, Track_num, DTW->leg, motion);
		...
		case 1:
			for(int j = 12; j >= 11; j--)//頭部
			{
				ColorBarElement(timeline, j, Track_num, DTW->head, motion);
				PartDepLine(timeline, 0, Track_num++, DTW, motion, pattern);
			}
			Track_num++;
			for(int j = 10; j >= 7; j--)//胸部
			{
				ColorBarElement(timeline, j, Track_num, DTW->chest, motion);
				PartDepLine(timeline, 1, Track_num++, DTW, motion, pattern);
			}
			ColorBarElement(timeline, 0, Track_num, DTW->chest, motion);//腰
			PartDepLine(timeline, 1, Track_num++, DTW, motion, pattern);
			Track_num++;
			for(int j = 13; j <= 16; j++)//右腕
			{
		...
		case 2:
			...
			for(int j = 13; j <= 16; j++)//右腕
			{
				ColorBarElement(timeline, j, Track_num, DTW->right_arm, motion);
				PartDepLine(timeline, 2, Track_num++, DTW, motion, pattern);
			}
			Track_num++;
			for(int j = 36; j <= 39; j++)//左腕
			{
				ColorBarElement(timeline, j, Track_num, DTW->left_arm, motion);
				PartDepLine(timeline, 3, Track_num++, DTW, motion, pattern);
			}
			Track_num++;
			for(int j = 1; j <= 3; j++)//右脚
		...
		case 3:
			...
			for(int j = 1; j <= 3; j++)//右脚
			{
				ColorBarElement(timeline, j, Track_num, DTW->right_leg, motion);
				PartDepLine(timeline, 4, Track_num++, DTW, motion, pattern);
			}
			Track_num++;
			for(int j = 4; j <= 6; j++)//左脚
			{
				ColorBarElement(timeline, j, Track_num, DTW->left_leg, motion);
				PartDepLine(timeline, 5, Track_num++, DTW, motion, pattern);
			}
			break;
		case 4:
		...
		case 5:
			...
			{
				if(view_segment != j)
					ColorBarElementGray(timeline, j, Track_num, DTW->DTWdistance[j], motion);
				else
					ColorBarElement(timeline, j, Track_num, DTW->DTWdistance[j], motion);
				Track_num++;
			}
			Track_num++;
			for(int j = 10; j >= 7; j--)//胸部
			...
			if(view_segment != 0)//腰
					ColorBarElementGray(timeline, 0, Track_num, DTW->DTWdistance[0], motion);
				else
					ColorBarElement(timeline, 0, Track_num, DTW->DTWdistance[0], motion);
			Track_num++;
			Track_num++;
			for(int j = 13; j <= 16; j++)//右腕
			{
...
//カラーバーの誤差による色の変化を設定
//
void MotionPlaybackApp::ColorBarElement(Timeline* timeline, int segment_num, int Track_num, float * frame_dep, Motion & motion)
{
...
//
void MotionPlaybackApp::PartDepLine(Timeline* timeline, int part_num, int Track_num, DTWinformation* DTW, Motion& motion, int pattern)
{
	float max_dep = 0.0f;
	float max_frame = -1.0f;
	float name_space = 30.0f;
	...
	if(part_num == 0)
		...
		department = DTW->chest;
	else if(part_num == 2)
		department = DTW->right_arm;
	else if(part_num == 3)
		department = DTW->left_arm;
	else if(part_num == 4)
		department = DTW->right_leg;
	else
		department = DTW->left_leg;
	...
	for(int i = 0; i < motion.num_frames; i++)
	{
		if(max_dep < department[i])
		{
			max_dep = department[i];
			max_frame = i;
		}
	}
				
	//誤差の最大値を色付けして表示
	timeline->AddSubElement( 1, max_frame + name_space, max_frame + 1.0f + name_space, Track_num * 1.2f, 1.0f + Track_num * 1.2f, Color4f( 0.5f, 0.0f, 1.0f, 0.2f ) );
}
...
float MotionPlaybackApp::DistanceCalc(int num_segments, vector<Vector3f> v1, vector<Vector3f> v2)
{
	...
	for(int i = 0; i < num_segments; i++)
	{
		...
		pos_des += sqrt(pow(v1[i].x - v2[i].x, 2.0) + pow(v1[i].y - v2[i].y, 2.0) + pow(v1[i].z - v2[i].z, 2.0));
	}
...
void DTWinformation::DTWinformation_init( int frames1, int frames2, const Motion & motion1, const Motion & motion2 )
{
	...
	this->DTWpass = new int * [motion1.body->num_segments];
	this->PassAll = new int[frames1];
	this->DTWdistance = new float * [motion1.body->num_segments];
	this->DistanceAll = new float[frames1];
	this->DTWorder = new int[motion1.body->num_segments];
	this->DTWforder = new int[motion1.body->num_segments];
	this->DTWtotal = 0.0f;
	this->left_leg = new float[frames1 + 1];
	this->right_leg = new float[frames1 + 1];
	this->chest = new float[frames1 + 1];
	this->head = new float[frames1 + 1];
	this->left_arm = new float[frames1 + 1];
	this->right_arm = new float[frames1 + 1];
	this->head_chest = new float[frames1 + 1];
	this->arm = new float[frames1 + 1];
	this->leg = new float[frames1 + 1];
	...
	{
		this->DTWpass[i] = new int[frames1];
		this->DTWpass[i][0] = 0;
		this->DTWdistance[i] = new float[frames1 + 1];
		this->DTWorder[i] = i;
		this->DTWforder[i] = i;
	}
	...
	{
		this->chest[i] = 0.0f;
		this->right_leg[i] = 0.0f;
		this->left_leg[i] = 0.0f;
		this->head[i] = 0.0f;
		this->right_arm[i] = 0.0f;
		this->left_arm[i] = 0.0f;
		this->head_chest[i] = 0.0f;
		this->arm[i] = 0.0f;
		this->leg[i] = 0.0f;
	}
			...
			for(int k = 0; k < frames2; k++)
			{
				for(int i = 0; i < motion1.body->num_segments; i++)
				{
					...
					if(min_distance > distance)
					{
						...
						this->DistanceAll[j] = pos_dis;
					}
				}
			}
		else
			for(int k = this->PassAll[j - 1]; k < frames2; k++)
			{
				for(int i = 0; i < motion1.body->num_segments; i++)
				{
					//手の部位は計算しない
	...
	for(int i = 0; i < motion1.body->num_segments; i++)
	{
		...
		for(int j = 0; j < frames1; j++)
		{
			if(j == 0)//部位の探索の最初にすること
			{
				...
				//{
				//	ForwardKinematics( motion1.frames[ s ], seg_frame_array0, joi_pos_array0 );
				//	mat0.transpose( seg_frame_array0[ i ] );
				//	v0 = {mat0.m30, mat0.m31, mat0.m32};
				//	for(int k = 0; k < frames2; k++)
				//	{
				//		//2つの部位の位置の距離、フレームの差をそれぞれ取り、その和を誤差(コスト)とする
				//		pos_dep = sqrt(pow(v0.x - v2[k].x, 2.0) + pow(v0.y - v2[k].y, 2.0) + pow(v0.z - v2[k].z, 2.0));
				//		frame_dep = 0.001f * abs(k - s);
				//		departure = pos_dep + frame_dep;
				//		//もし一番距離が小さいパスのコストより小さければ置き換える
				//		if(min_departure > departure)
				//		{
				//			this->DTWpass[i][s] = k;
				//			min_departure = departure;
				//			this->DTWdepartment[i][s] = pos_dep;
				//		}
				//	}
				//	if(s != 0)
				//		for(int m = s - frame_ival + 1; m < s; m++)
				//			this->DTWpass[i][m] = this->DTWpass[i][s];
				//	if(s != 0 && s + frame_ival >= frames1)
				//	{
				//		for(int m = s; m < frames1; m++)
				//			this->DTWpass[i][m] = frames2 - 1;
				//		break;
				//	}
				//	std::cout << "flag";
				...
				for(int k = 0; k < frames2; k++)
				{
					...
					if(min_distance > distance)
					{
						this->DTWpass[i][j] = k;
						min_distance = distance;
						this->DTWdistance[i][j] = pos_dis;
					}
				}
			}

			if(j != 0)
				for(int k = this->DTWpass[i][j - 1]; k < frames2; k++)
				{
					//if(j % frame_ival == 0)
					//	break;
					...
					if(min_distance > distance)
					{
						this->DTWpass[i][j] = k;
						min_distance = distance;
						this->DTWdistance[i][j] = min_distance;
					}
				}
			...
			//{
			//	this->DTWpass[i][j] = this->DTWpass[i][j-1];
			//	this->DTWdepartment[i][j] = sqrt(pow(v1.x - v2[this->DTWpass[i][j]].x, 2.0) + pow(v1.y - v2[this->DTWpass[i][j]].y, 2.0) + pow(v1.z - v2[this->DTWpass[i][j]].z, 2.0));
			//}

			//jのフレームのパスaと、j+1のフレームのパスbがあったとき、a>bとなると時間が逆になってしまうためそれを避ける
			//if(j < frames1 - 1 && this->DTWpass[i][j] > this->DTWpass[i][j+1])
			//{
			//	this->DTWpass[i][j] = this->DTWpass[i][j+1];
			//	this->DTWdepartment[i][j] = sqrt(pow(v1.x - v2[this->DTWpass[i][j]].x, 2.0) + pow(v1.y - v2[this->DTWpass[i][j]].y, 2.0) + pow(v1.z - v2[this->DTWpass[i][j]].z, 2.0));
			//}
			...
			{
				std::cout << i << std::endl;
				for(int m = 0; m < frames1; m++ )
					std::cout << m << " -> " << DTWpass[i][m] << " ";
				 std::cout << std::endl;
			}
			...
			if(i == 0)
			{
				this->chest[j] += this->DTWdistance[i][j];
				this->head_chest[j] += this->DTWdistance[i][j];
			}
			else if( i <= 3 )
			{
				this->right_leg[j] += this->DTWdistance[i][j];
				this->leg[j] += this->DTWdistance[i][j];
			}
			else if( i <= 6 )
			{
				this->left_leg[j] += this->DTWdistance[i][j];
				this->leg[j] += this->DTWdistance[i][j];
			}
			else if( i <= 10 )
			{
			...
			{
				this->head[j] += this->DTWdistance[i][j];
				this->head_chest[j] += this->DTWdistance[i][j];
			}
			else if( i <= 35 )
			{
				this->right_arm[j] += this->DTWdistance[i][j];
				this->arm[j] += this->DTWdistance[i][j];
			}
			else
			{
				this->left_arm[j] += this->DTWdistance[i][j];
				this->arm[j] += this->DTWdistance[i][j];
			}
	...
	{
		this->DTWdistance[i][frames1] = 0.0f;
		if(i <= 16)
			for(int j = 0; j < frames1; j++)
				this->DTWdistance[i][frames1] += this->DTWdistance[i][j];
		else if(i >= 36 && i <= 39)
			for(int j = 0; j < frames1; j++)
				this->DTWdistance[i][frames1] += this->DTWdistance[i][j];
	}
	...
	for(int i = 0; i < motion1.body->num_segments - 1; i++)
		for(int j = i; j < motion1.body->num_segments; j++)
			if(this->DTWdistance[this->DTWorder[i]][frames1] < this->DTWdistance[this->DTWorder[j]][frames1])
	...
	{
		//全部位の誤差の合計を計算
		this->DTWtotal += this->DTWdistance[i][frames1];

		//誤差の合計を部位ごとに加算
		if(i == 0)
			this->chest[frames1] += this->DTWdistance[i][frames1];
		else if( i <= 3 )
			this->right_leg[frames1] += this->DTWdistance[i][frames1];
		else if( i <= 6 )
			this->left_leg[frames1] += this->DTWdistance[i][frames1];
		else if( i <= 10 )
			this->chest[frames1] += this->DTWdistance[i][frames1];
		else if( i <= 12 )
			this->head[frames1] += this->DTWdistance[i][frames1];
		else if( i <= 35 )
			this->right_arm[frames1] += this->DTWdistance[i][frames1];
		else
			this->left_arm[frames1] += this->DTWdistance[i][frames1];

		std::cout << " [ " << this->DTWorder[i] << " segment: " << this->DTWdistance[this->DTWorder[i]][frames1] << " ] ";
	}
	std::cout << std::endl;
}
...
void DTWinformation::DTWinformation_frame(int now_frame, int num_segments)
{
	for(int i = 0; i < num_segments - 1; i++)
	{
		for(int j = i; j < num_segments; j++)
			if(this->DTWdistance[this->DTWforder[i]][now_frame] < this->DTWdistance[this->DTWforder[j]][now_frame])
...
//			// 体節の中心の位置・向きを基準とする変換行列を適用
//			mat1.transpose( seg_frame_array1[ i ] );
//			v1 = {mat1.m30, mat1.m31, mat1.m32};
//
//			if(j == 0) //最初に１部位に対する全フレームの位置を格納する
//				for(int k = 0; k < frames2; k++)
//				{
//					ForwardKinematics( motion2.frames[ k ], seg_frame_array2, joi_pos_array2 );
//					mat2.transpose( seg_frame_array2[ i ] );
//					v2[k] = {mat2.m30, mat2.m31, mat2.m32};
//				}
//
//			for(int k = 0; k < frames2; k++)
//			{
...
//			}
//
//			//j番目のフレームに対応するa番目と、j-1番目のフレームに対応するb番目があったとき、a<bとなると時間が逆になってしまうためそれを避ける
//			if(j > 0 && this->DTWlabel[j] < this->DTWlabel[j-1])
//			{
//				this->DTWlabel[j] = this->DTWlabel[j-1];
//				this->DTWdepartment[i][j] = sqrt(pow(v1.x - v2[this->DTWlabel[j]].x, 2.0) + pow(v1.y - v2[this->DTWlabel[j]].y, 2.0) + pow(v1.z - v2[this->DTWlabel[j]].z, 2.0));
//			}
//
//			//誤差の合計を部位ごとに加算
//			if(i == 0)
//			{
//				this->chest[j] += this->DTWdepartment[i][j];
//				this->head_chest[j] += this->DTWdepartment[i][j];
//			}
//			else if( i <= 3 )
//			{
//				this->right_leg[j] += this->DTWdepartment[i][j];
//				this->leg[j] += this->DTWdepartment[i][j];
//			}
//			else if( i <= 6 )
//			{
//				this->left_leg[j] += this->DTWdepartment[i][j];
//				this->leg[j] += this->DTWdepartment[i][j];
//			}
//			else if( i <= 10 )
//			{
...
//			else if( i <= 12 )
//			{
//				this->head[j] += this->DTWdepartment[i][j];
//				this->head_chest[j] += this->DTWdepartment[i][j];
//			}
//			else if( i <= 35 )
//			{
//				this->right_arm[j] += this->DTWdepartment[i][j];
//				this->arm[j] += this->DTWdepartment[i][j];
//			}
//			else
//			{
//				this->left_arm[j] += this->DTWdepartment[i][j];
//				this->arm[j] += this->DTWdepartment[i][j];
//			}
//
//			min_departure = 100.0f;
...
////パス作成
// {
//			if(j > 0 && this->DTWlabel[j-1] < frames2 - 1)
//			{
//				int k = this->DTWlabel[j-1];
//				pos_dep = sqrt(pow(v1.x - v2[k].x, 2.0) + pow(v1.y - v2[k].y, 2.0) + pow(v1.z - v2[k].z, 2.0));
//				frame_dep = 0.004f * abs(k - j);
//				departure = pos_dep + frame_dep;
//				pos_dep2 = sqrt(pow(v1.x - v2[k + 1].x, 2.0) + pow(v1.y - v2[k + 1].y, 2.0) + pow(v1.z - v2[k + 1].z, 2.0));
//				frame_dep2 = 0.004f * abs(k + 1 - j);
//				departure2 = pos_dep2 + frame_dep2;
...
//				else
//				{
//					this->DTWdepartment[i][j] = pos_dep2;
//					this->DTWlabel[j] = k + 1;
//				}
//			}
//			else if(j > 0)
...
//			else
//			{
//				pos_dep = sqrt(pow(v1.x - v2[0].x, 2.0) + pow(v1.y - v2[0].y, 2.0) + pow(v1.z - v2[0].z, 2.0));
//				this->DTWdepartment[i][j] = pos_dep;
//				this->DTWlabel[j] = 0;
//			}
//}